						...
						>
							...
							<tbody>
								<tr v-for="subject in subject" :key="subject.id">
									...
									<td>
										<nuxt-link
											:to="`/institute/subject/${topic.id}/edit`"
										...
										<button
											...
											@click.prevent="deletesubject(subject.id, index)"
										>
											Delete
										</button>
									</td>
		...
		async getsubjectIndex() {
			try {
				const res = await this.$axios.get(
					`/institute/subject/index/?page=${this.page}`
				);
				this.sjubject = res.data.subject.data ?? [];
				this.subject = res.data.subject.total ?? 1;
				this.per_page = res.data.subject.per_page ?? 20;
			} catch (err) {
				console.log(err);
			}
		},
			...
			try {
				...
				if (confirm_referal_code_delete) {
					const res = await this.$axios.post(
						`/institute/subject/delete/${topic_id}`,
						{
							_method: 'DELETE',
						}
					);
